...
list_t  *g_pstPtsNendRecvNode = NULL;
list_t  *g_pstPtsNendSendNode = NULL;
PTS_SERV_MSG_ST g_stPtsMsg;
sem_t g_SemPts;
sem_t g_SemPtsRecv;
PTS_SERV_SOCKET_ST g_lPtsServSocket[PTS_WEB_SERVER_MAX_SIZE];
U32 g_ulCreateSocketCount = 0;
static S32 g_lPtsUdpSocket = 0;
...
{
     S8 *aucDestID = (S8 *)para;
     dos_strncpy(aucDestID, column_value[0], PTC_ID_LEN);
     aucDestID[PTC_ID_LEN] = '\0';
     return DOS_SUCC;
}
...
S32 pts_find_ptc_by_dest_addr(S8 *pDestInternetIp, S8 *pDestIntranetIp, S8 *aucDestID)
{
    ...
    dos_memzero(aucDestID, PTC_ID_LEN+1);
    sprintf(achSql, "select sn from ipcc_alias where register = 1 and internetIP='%s' and intranetIP='%s'", pDestInternetIp, pDestIntranetIp);

    dos_sqlite3_exec_callback(g_stMySqlite, achSql, pts_find_ptc_callback, (VOID *)aucDestID);
    if (dos_strlen(aucDestID) > 0)
    {
        return DOS_SUCC;
    }
    else
    {
        sprintf(achSql, "select sn from ipcc_alias where register = 1 and internetIP='%s'", pDestInternetIp);
        dos_sqlite3_exec_callback(g_stMySqlite, achSql, pts_find_ptc_callback, (VOID *)aucDestID);
        if (dos_strlen(aucDestID) > 0)
        {
            return DOS_SUCC;
...
VOID pts_data_lose(PT_MSG_TAG *pstMsgDes, S32 lLoseSeq)
{
    ...
    //printf("send lose data, type = %d, stream = %d, seq = %d\n", pstMsgDes->enDataType, pstMsgDes->ulStreamID, lLoseSeq);
    pt_logr_info("send lose data, type = %d, stream = %d, seq = %d", pstMsgDes->enDataType, pstMsgDes->ulStreamID, lLoseSeq);
...
VOID pts_send_lost_data_req(U64 ulLoseMsg)
{
    ...
    pt_logr_info("pstStreamNode->ulCountResend = %d , stream = %d", pstStreamNode->ulCountResend, pstStreamNode->ulStreamID);

    if (pstStreamNode->ulCountResend >= 3)
    {
        /* TODO 3秒后，未收到包。关闭定时器、sockfd */
    ...
    pt_logr_info("send lose data count : %d", ulCount);
...
VOID pts_handle_logout_req(PT_MSG_TAG *pstMsgDes)
{
    ...
    /* 通知pts，修改数据库 */
    sprintf(achSql, "update ipcc_alias set register=0 where sn='%.*s';", PTC_ID_LEN, pstMsgDes->aucID);
...
VOID pts_handle_login_req(S32 lSockfd, PT_MSG_TAG *pstMsgDes, struct sockaddr_in stClientAddr, S8 *szPtcVersion)
{
    ...
    dos_memcpy(acBuff, (VOID *)&stCtrlData, sizeof(PT_CTRL_DATA_ST));

    lResult = pts_save_into_send_cache(pstPtcNode, PT_CTRL_LOGIN_RSP, pstMsgDes->enDataType, acBuff, sizeof(PT_CTRL_DATA_ST), NULL, 0);
    if (lResult == DOS_SUCC)
    {
...
/*VOID pts_send_hb_rsp(PT_MSG_TAG *pstMsgDes)
{
    ...
    dos_memcpy(acBuff, (VOID *)&stLoginRes, sizeof(PT_CTRL_DATA_ST));
    printf("!!!!!!!!!!!!!!send hb\n");
    //pts_save_msg_into_cache(pstMsgDes->aucID, PT_DATA_CTRL, PT_CTRL_HB_RSP, acBuff, sizeof(PT_CTRL_DATA_ST));
...
VOID pts_send_hb_rsp(S32 lSockfd, PT_MSG_TAG *pstMsgDes, struct sockaddr_in stClientAddr)
{
    ...
    dos_memcpy(acBuff+sizeof(PT_MSG_TAG), (VOID *)&stLoginRes, sizeof(PT_CTRL_DATA_ST));

    sendto(lSockfd, acBuff, sizeof(PT_CTRL_DATA_ST) + sizeof(PT_MSG_TAG), 0, (struct sockaddr*)&stClientAddr, sizeof(stClientAddr));
...
VOID pts_send_exit_notify_to_ptc(PT_MSG_TAG *pstMsgDes, PT_CC_CB_ST *pstPtcSendNode)
{
    ...
    sendto(pstPtcSendNode->lSocket, acBuff, sizeof(PT_MSG_TAG), 0, (struct sockaddr*)&pstPtcSendNode->stDestAddr, sizeof(pstPtcSendNode->stDestAddr));
    return;
}
...
VOID pts_hd_timeout_callback(U64 param)
{
    ...
    pt_logr_debug("hd rsp timeout : %d", pstPtcRecvNode->usHBOutTimeCount);
    if (pstPtcRecvNode->usHBOutTimeCount > PTS_HB_TIMEOUT_COUNT_MAX)
    {
        /* 连续多次无法收到心跳，ptc掉线 */
        pt_logr_info("ptc lost connection!");
        dos_tmr_stop(&pstPtcRecvNode->stHBTmrHandle);
        ...
        /* 修改数据库 */
        sprintf(achSql, "update ipcc_alias set register=0 where sn='%.*s';", PTC_ID_LEN, aucID);
        dos_sqlite3_exec(g_stMySqlite, achSql);
    }
}
...
VOID pts_ctrl_msg_handle(S32 lSockfd, S8 *pData, struct sockaddr_in stClientAddr)
{
    ...
    switch (pstCtrlData->enCtrlType)
    {
        ...
        case PT_CTRL_PTS_MAJOR_DOMAIN:
            printf("achPtsMajorDomain : %s\n\n", pstCtrlData->achPtsMajorDomain);
            if (pstCtrlData->achPtsMajorDomain[0] != '\0')
            {
                sprintf(achSql, "update ipcc_alias set achPtsMajorDomain='%s' where sn='%.*s';", pstCtrlData->achPtsMajorDomain, PTC_ID_LEN, pstMsgDes->aucID);
                dos_sqlite3_exec(g_stMySqlite, achSql);
            }
            if (pstCtrlData->usPtsMajorPort != 0)
            {
                sprintf(achSql, "update ipcc_alias set usPtsMajorPort=%d where sn='%.*s';", dos_ntohs(pstCtrlData->usPtsMajorPort), PTC_ID_LEN, pstMsgDes->aucID);
                dos_sqlite3_exec(g_stMySqlite, achSql);
            }
            break;
        case PT_CTRL_PTS_MINOR_DOMAIN:
            printf("achPtsMinorDomain : %s\n\n", pstCtrlData->achPtsMinorDomain);
            ...
            if (pstNeedSendNode->bIsResend)
            {
                ...
                else
                {
                    ...
                    while (ulSendCount)
                    {
                        ...
                        sendto(lSockfd, acBuff, stSendDataNode.ulLen + sizeof(PT_MSG_TAG), 0, (struct sockaddr*)&pstPtcNode->stDestAddr, sizeof(pstPtcNode->stDestAddr));
                        ulSendCount--;
                    }
        ...
        if (FD_ISSET(lSockfd, &ReadFdsCpio))
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        for (i=sizeof(PT_MSG_TAG); i<lRecvLen; i++)
                        {
                            if (i%16 == 0)
                            {
                                printf("\n");
                            }
                            printf("%02x ", (U8)(acRecvBuf[i]));
                        }
                        printf("\n-------------------------------------\n");
                    }
#endif

                    pt_logr_info("pts recv data from ptc, seq : %d, streamID = %d, lRecvLen = %d", pstMsgDes->lSeq, pstMsgDes->ulStreamID, lRecvLen);
