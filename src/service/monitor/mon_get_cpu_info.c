...
static U32  mon_get_cpu_data(MON_SYS_CPU_TIME_S * pstCpu)
{
   ...
   if (NULL != (fgets(szBuff, MAX_BUFF_LENGTH, fp)))
   {
      ...
      if(DOS_SUCC != ulRet)
      {
         mon_trace(MON_TRACE_CPU, LOG_LEVEL_ERROR, "Analyse buffer by regular expression FAIL.");
         goto failure;
      }
...
U32 mon_init_cpu_queue()
{
   ...
   dos_memzero(m_pstCPUQueue, sizeof(MON_CPU_QUEUE_S));
   ...
   if(DOS_ADDR_INVALID(m_pstCPUQueue->pstHead))
   {
      ...
      if(DOS_SUCC != ulRet)
      {
         mon_trace(MON_TRACE_CPU, LOG_LEVEL_ERROR, "CPU Module Alloc Memory FAIL.");
         return DOS_FAIL;
      }
   ...
   while ((m_pstCPUQueue->ulQueueLength) < TEN_MINUTE_SAMPLE_ARRAY)
   {
      ...
      if(DOS_SUCC != ulRet)
      {
          mon_trace(MON_TRACE_CPU, LOG_LEVEL_ERROR, "CPU Node Insert into Queue FAIL.");
          return DOS_FAIL;
      }
...
U32  mon_get_cpu_rslt_data()
{
    ...
    */
    ...
    if (DOS_SUCC != ulRet)
    {
        mon_trace(MON_TRACE_CPU, LOG_LEVEL_ERROR, "Refresh CPU Queue FAIL.");
        return DOS_FAIL;
    }
...
static U32  mon_refresh_cpu_queue()
{
    ...
    if(DOS_SUCC != ulRet)
    {
        mon_trace(MON_TRACE_CPU, LOG_LEVEL_ERROR, "Get CPU timestamp Data FAIL.");
        return DOS_FAIL;
    }
    ...
    if(DOS_SUCC != ulRet)
    {
        mon_trace(MON_TRACE_CPU, LOG_LEVEL_ERROR, "CPU data node EnQueue FAIL.");
        return DOS_FAIL;
    }
    ...
    if(DOS_SUCC != ulRet)
    {
        mon_trace(MON_TRACE_CPU, LOG_LEVEL_ERROR, "CPU data node DeQueue FAIL.");
        return DOS_FAIL;
    }
