    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            if (DOS_ADDR_INVALID(pstLogFile))
            {
                ...
                if (DOS_ADDR_INVALID(pstLogFile))
                {
                    sc_log(LOG_LEVEL_ERROR, "Create Log File FILE. %s", strerror(errno));
                    sleep(2);
                    break;
                }
            ...
            {
                sc_log(LOG_LEVEL_ERROR, "%s", "Log digest error.");
                break;
            }

            if (DOS_ADDR_INVALID(pstDLLNode->pHandle))
            {
                sc_log(LOG_LEVEL_ERROR, "%s", "Log digest is empty.");
                break;
            }
            ...
            {
                sc_log(LOG_LEVEL_ERROR, "%s", "Write into file FILE.");
            }
            ...
            if (ulFileSize > SC_LOG_DIGEST_FILE_MAX_SIZE)
            {
                ...
                {
                    sc_log(LOG_LEVEL_DEBUG, "execute %s FAIL!!!", szPsCmd);
                }
            }
        }
    }
...
U32 sc_log_digest_print(S8 *pszFormat, ...)
{
    ...
    {
        sc_log(LOG_LEVEL_ERROR, "%s", "Add log digest fail. Alloc memory fail");
        return DOS_FAIL;
    }
...
U32 sc_log_digest_start()
{
    ...
    {
        sc_log(LOG_LEVEL_NOTIC, "%s", "Create Log digest process pthread fail");
