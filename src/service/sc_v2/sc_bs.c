...
U32 sc_bs_auth_rsp_proc(BS_MSG_TAG *pstMsg)
{
    ...
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_ERROR, SC_MOD_BS), "%s", "Invalid Msg.");
        return DOS_FAIL;
    }

    if (pstMsg->usMsgLen != sizeof(BS_MSG_AUTH))
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_ERROR, SC_MOD_BS), "Invalid Msg length. Length: %d", pstMsg->usMsgLen);
        return DOS_FAIL;
    }
    ...
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_ERROR, SC_MOD_BS), "%s", "Invalid Msg body. ");
        return DOS_FAIL;
    }
    ...
    stEvtAuth.ulSessionNum = pstAuthMsg->ulSessionNum;

    ulRet = sc_send_event_auth_rsp(&stEvtAuth);

    sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_DEBUG, SC_MOD_BS), "Process auth response finished. CR:%u SCB: %u", pstLegCB->ulSCBNo, pstLegCB->ulSCBNo);
...
U32 sc_bs_billing_start_rsp_proc(BS_MSG_TAG *pstMsg)
{
    ...
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_ERROR, SC_MOD_BS), "Invalid RC NO. Max: %d, Current: %s", SC_SCB_SIZE, pstMsg->ulCRNo);
        return DOS_FAIL;
    }
    ...
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_ERROR, SC_MOD_BS), "%s", "Invalid SCB.");
        return DOS_FAIL;
    }

    sc_log(pstSCB->bTrace, SC_LOG_SET_MOD(LOG_LEVEL_DEBUG, SC_MOD_BS), "%s", "Start process billing start response msg.");

    sc_log(pstSCB->bTrace, SC_LOG_SET_MOD(LOG_LEVEL_DEBUG, SC_MOD_BS), "%s", "Process billing start response msg finished.");
...
U32 sc_bs_billing_update_rsp_proc(BS_MSG_TAG *pstMsg)
{
    ...
    if (pstMsg->ulCRNo >= SC_SCB_SIZE)
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_ERROR, SC_MOD_BS), "%s", "Invalid RC NO. Max: %d, Current: %s", SC_SCB_SIZE, pstMsg->ulCRNo);
        return DOS_FAIL;
    }
    ...
    sc_log(pstSCB->bTrace, SC_LOG_SET_MOD(LOG_LEVEL_DEBUG, SC_MOD_BS), "%s", "Start process billing update response msg.");

    sc_log(pstSCB->bTrace, SC_LOG_SET_MOD(LOG_LEVEL_DEBUG, SC_MOD_BS), "%s", "Process billing update response msg finished.");
...
U32 sc_bs_billing_stop_rsp_proc(BS_MSG_TAG *pstMsg)
{
    ...
    sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_DEBUG, SC_MOD_BS), "Process billing stop response msg finished.ERRNO: %d", pstMsg->ucErrcode);
...
VOID sc_bs_hb_timeout(U64 uLIndex)
{
    ...
    if (g_pstSCBSClient[uLIndex]->ulHBFailCnt > SC_BS_HB_FAIL_CNT)
    {
        ...
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_NOTIC, SC_MOD_BS), "BS Server Lost. Index:%u", uLIndex);
...
VOID sc_bs_hb_interval_timeout(U64 uLIndex)
{
    ...
    sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_DEBUG, SC_MOD_BS), "Heartbeat between BS and SC interval timeout. Client Index : %u", uLIndex);
...
VOID sc_bs_msg_proc(U8 *pData, U32 ulLength, U32 ulClientIndex)
{
    ...
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_WARNING, SC_MOD_BS), "Recv Invalid msg. Length:%s", ulLength);
        return;
    }

    if (ulClientIndex >= SC_MAX_BS_CLIENT)
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_WARNING, SC_MOD_BS), "Invalid clinet index. Index:%X", ulClientIndex);
        return;
    }
    ...
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_WARNING, SC_MOD_BS), "Recv Invalid msg. Length:%s, Length in Msg", ulLength, pstMsgHeader->usMsgLen);
        return;
    }

    sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_DEBUG, SC_MOD_BS), "Recv BS Msg. Version: %X, Port:%d, IP:%X, Seq:%d, CR No:%d, Msg Type:%d, ErrCode: %d, Msg Length:%d"
    ...
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_WARNING, SC_MOD_BS), "Recv Invalid msg. Version not match. Version:%X, Version in Msg", BS_MSG_INTERFACE_VERSION, pstMsgHeader->usVersion);
        return;
    }
    ...
    switch (pstMsgHeader->ucMsgType)
    {
        ...
        default:
            sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_WARNING, SC_MOD_BS), "Recv Invalid msg. Templetely not support. %d", pstMsgHeader->ucMsgType);
            break;
    }
}
...
U32 sc_bs_connect(U32 ulIndex)
{
    ...
    if (lRet < 0)
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_WARNING, SC_MOD_BS), "%s", "Bind socket fail.");
        close(g_pstSCBSClient[ulIndex]->lSocket);
        return DOS_FAIL;;
    }

    sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_DEBUG, SC_MOD_BS), "Bind socket to addr: %s, Len: %d.", g_pstSCBSClient[0]->stAddr.stUnAddr.sun_path, lAddrLen);
...
VOID *sc_bs_recv_mainloop(VOID *ptr)
{
    ...
    while (1)
    {
        ...
        {
            sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_ERROR, SC_MOD_BS), "%s", "All the BS server Lost, re-connect after 1 second");
        ...
        for (ulActiveClientCnt = 0, ulIndex = 0; ulIndex < SC_MAX_BS_CLIENT; ulIndex++)
        {
            ...
            if (DOS_ADDR_INVALID(pBuffer))
            {
                ...
                sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_EMERG, SC_MOD_BS), "Disconnect BS for alloc memory FAIL. Index: %u", ulIndex);
            ...
            if (lRet < 0)
            {
                ...
                sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_NOTIC, SC_MOD_BS), "%s", "Recv data from socket fail.");
                dos_dmem_free(pBuffer);
                pBuffer = NULL;
                continue;
            }
            ...
            {
                sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_NOTIC, SC_MOD_BS), "Recv invalid data from socket fail. Length not complete. Length: %d", lRet);
                dos_dmem_free(pBuffer);
                pBuffer = NULL;
                continue;
            }
            ...
            if (DOS_ADDR_INVALID(pstDLLNode))
            {
                ...
                sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_NOTIC, SC_MOD_BS), "%s", "Discard msg FROM bs for alloc mem FAIL.");
                dos_dmem_free(pBuffer);
                pBuffer = NULL;
                continue;
            }
...
U32 sc_bs_fsm_start()
{
    ...
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_NOTIC, SC_MOD_BS), "%s", "Create BS MSG process pthread fail");
        DOS_ASSERT(0);
    ...
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_NOTIC, SC_MOD_BS), "%s", "Create BS MSG recv pthread fail");
        DOS_ASSERT(0);
...
U32 sc_bs_fsm_init()
{
    ...
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_NOTIC, SC_MOD_BS), "%s", "Create the hash table fail for the BS MSG.");
        return DOS_FAIL;
    }
    ...
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_NOTIC, SC_MOD_BS), "%s", "Alloc memory fail for the BS Client CB.");
        return DOS_FAIL;
    }
