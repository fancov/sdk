...
static VOID *sc_pub_runtime(VOID *ptr)
{
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            {
                sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_INFO, SC_MOD_PUBLISH), "%s", "DLL Node is null, may be the queue is breaken");
                break;
            }
            ...
            {
                sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_INFO, SC_MOD_PUBLISH), "%s", "DLL Node with empty data.");
                continue;
            }

            if (DOS_ADDR_INVALID(pstPubData->pszURL))
            {
                sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_INFO, SC_MOD_PUBLISH), "%s", "Publish request is invalid.");
...
static VOID *sc_pub_runtime_master(VOID *ptr)
{
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            if (DOS_ADDR_INVALID(pstDllNode))
            {
                sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_WARNING, SC_MOD_PUBLISH), "%s", "DLL Node is null, may be the queue is breaken");
                break;
            }
            ...
            if (DOS_ADDR_INVALID(pstPubData))
            {
                sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_WARNING, SC_MOD_PUBLISH), "%s", "DLL Node with empty data, may be the queue is breaken");
...
U32 sc_pub_send_msg(S8 *pszURL, S8 *pszData, U32 ulType, VOID *pstData)
{
    ...
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_ERROR, SC_MOD_PUBLISH), "%s", "Invalid msg type.(%u)", ulType);
        return DOS_FAIL;
    }
    ...
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_ERROR, SC_MOD_PUBLISH), "%s", "Alloc memory fail while send publish request.");
    ...
    if (DOS_ADDR_INVALID(pstPubData->pszURL))
    {
        sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_ERROR, SC_MOD_PUBLISH), "%s", "Dump the url fail.");
        goto proc_fail;
    }
    ...
    if (DOS_ADDR_VALID(pszData))
    {
        ...
        {
            sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_ERROR, SC_MOD_PUBLISH), "%s", "Dump the data fail.");
            goto proc_fail;
        }
    }
    ...
    sc_log(DOS_FALSE, SC_LOG_SET_MOD(LOG_LEVEL_DEBUG, SC_MOD_PUBLISH), "Publish request send succ. URL: %s, Data: %s", pszURL, DOS_ADDR_VALID(pszData) ? pszData : "");
