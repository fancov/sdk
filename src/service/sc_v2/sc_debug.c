...
VOID sc_show_agent(U32 ulIndex, U32 ulID, U32 ulCustomID, U32 ulGroupID)
{
    ...
    cli_out_string(ulIndex, szCmdBuff);
    dos_snprintf(szCmdBuff, sizeof(szCmdBuff)
                    , "\r\n%5s%10s%10s%8s%10s%8s%8s%8s%8s%7s%8s%12s%12s%12s%9s%10s%14s%12s%12s%9s%9s%5s%8s%13s%12s%10s"
                    , "ID", "w-Status", "s-Status", "NeedCon", "Connected", "Custom", "Group1", "Group2"
                    , "Record", "Trace", "Leader", "SIP Acc", "Extension", "Emp NO.", "CallCnt", "Bind", "Telephone", "Mobile", "TT_number", "Sip ID", "ScbNO", "bDel", "bLogin", "LastCustomer", "ProcessTime", "LegNo");
    cli_out_string(ulIndex, szCmdBuff);
    ...
    HASH_Scan_Table(g_pstAgentList, ulHashIndex)
    {
        HASH_Scan_Bucket(g_pstAgentList, ulHashIndex, pstHashNode, HASH_NODE_S*)
        {
            ...
            dos_snprintf(szCmdBuff, sizeof(szCmdBuff)
                        , "\r\n%5u%10s%10s%8s%10s%8u%8u%8u%8s%7s%8s%12s%12s%12s%9u%10s%14s%12s%12s%9u%9u%5s%8s%13s%12u%10u"
                        , pstAgentQueueNode->pstAgentInfo->ulAgentID
                        , sc_translate_agent_status(pstAgentQueueNode->pstAgentInfo->ucWorkStatus)
                        , sc_translate_agent_serv_status(pstAgentQueueNode->pstAgentInfo->ucServStatus)
