...
U32 sc_http_api_callerset_action(list_t *pstArgv)
{
    ...
    {
        sc_log(LOG_LEVEL_INFO, "%s", "CallerGrp Data is Synchronizing");
    }
    else
    {
        if (dos_atoul(pszCallerSetID, &ulSettingID) < 0)
        {
            sc_log(LOG_LEVEL_INFO, "%s", "Invalid parameters.");
            goto invalid_params;
        }
    }
    ...
    {
        sc_log(LOG_LEVEL_INFO, "Update Caller Setting FAIL.(ulAction:%u, ulID:%u)"
...
U32 sc_http_api_gateway_action(list_t *pstArgv)
{
    ...
    if (DOS_ADDR_INVALID(pszGateWayID)
        || DOS_ADDR_INVALID(pszAction))
    {
        sc_log(LOG_LEVEL_INFO, "%s", "Gateway data is synchronizing...");
    }
    else
    {
        if (dos_atoul(pszGateWayID, &ulGatewayID) < 0)
...
U32 sc_http_api_call_ctrl(list_t *pstArgv)
{
    ...
    sc_log(LOG_LEVEL_INFO, "Recv HTTP API CMD. CMD: %u, Action: %u, Customer: %u, Task: %u, Caller: %s"
...
U32 sc_http_api_agent(list_t *pstArgv)
{
    ...
    sc_log(LOG_LEVEL_INFO, "Recv HTTP API CMD. Action: %u, AgentID: %u, sip_userid: %s"
...
U32 sc_http_api_route_action(list_t *pstArgv)
{
    ...
    if (DOS_ADDR_INVALID(pszRouteID)
        || DOS_ADDR_INVALID(pszAction))
    {
        sc_log(LOG_LEVEL_INFO, "%s", "Route data is synchronizing...");
    }
    else
    {
...
U32 sc_http_api_black_action(list_t *pstArgv)
{
    ...
    sc_log(LOG_LEVEL_DEBUG, "Recv HTTP API CMD. CMD: %u, Action: %u, ulBlackID: %u"
                    , ulAction, ulBlackID);
...
U32 sc_http_api_process(SC_HTTP_CLIENT_CB_S *pstClient)
{
    ...
    sc_log(LOG_LEVEL_DEBUG, "HTTP Request: %s", pstClient->stDataBuff.pszBuff);
    ...
    sc_log(LOG_LEVEL_DEBUG, "%s", "Parse the http request finished.");
    ...
    sc_log(LOG_LEVEL_NOTIC, "HTTP Request process finished. Return code: %d", ulRet);
