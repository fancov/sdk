...
U32 sc_ep_terminate_call(SC_SCB_ST *pstSCB)
{
    ...
    if (DOS_ADDR_VALID(pstSCBOther))
    {
        ...
        {
            //sc_ep_esl_execute("hangup", NULL, pstSCB->szUUID);
            if (sc_ep_hangup_call(pstSCBOther, pstSCBOther->usTerminationCause) == DOS_SUCC)
            {
    ...
    if ('\0' != pstSCB->szUUID[0])
    {
        /* 有FS通讯的话，就直接挂断呼叫就好 */
        //sc_ep_esl_execute("hangup", NULL, pstSCB->szUUID);
        sc_ep_hangup_call(pstSCB, pstSCB->usTerminationCause);
    ...
    if (pstSCBPublish->ucStatus < SC_SCB_ACTIVE)
    {
        if (SC_SCB_ACTIVE == pstSCBSubscription->ucStatus)
        {
                ...
                || sc_call_check_service(pstSCBPublish, SC_SERV_ATTEND_TRANSFER))
            {
                ...
                sc_acd_agent_update_status(pstSCBNotify, SC_ACD_IDEL, U32_BUTT);
...
U32 sc_ep_transfer_publish_release(SC_SCB_ST * pstSCBPublish)
{
    ...
    sc_logr_debug(SC_ESL, "!!!!!!!!!!!!pstSCBNotify->ucStatus : %u, pstSCBSubscription->ucStatus : %u, pstSCBSubscription->usHoldCnt : %u", pstSCBNotify->ucStatus, pstSCBSubscription->ucStatus, pstSCBSubscription->usHoldCnt);
...
U32 sc_ep_call_agent(SC_SCB_ST *pstSCB, SC_ACD_AGENT_INFO_ST *pstAgentInfo, BOOL bIsUpdateCaller, U32 *pulErrCode)
{
    ...
    if (pstAgentInfo->bConnected && pstAgentInfo->usSCBNo < SC_MAX_SCB_NUM)
    {
        ...
        /* 更新坐席的状态 */
        sc_acd_agent_update_status(pstSCBNew, SC_ACD_BUSY, pstSCBNew->usSCBNo);
...
U32 sc_ep_call_ctrl_call_agent(U32 ulCurrentAgent, U32 ulAgentCalled)
{
    ...
    if (stAgentInfo.bConnected && stAgentInfo.usSCBNo < SC_MAX_SCB_NUM)
    {
        ...
        /* 更新坐席的状态 */
        sc_acd_agent_update_status(pstSCBOther, SC_ACD_BUSY, pstSCBOther->usSCBNo);
    ...
    else
    {
        ...
        switch (stAgentInfo.ucBindType)
        {
            ...
            case AGENT_BIND_MOBILE:
                ...
                break;
            case AGENT_BIND_TT_NUMBER:
                dos_strncpy(pstSCB->szCalleeNum, stCalleeAgentInfo.szTTNumber, sizeof(pstSCB->szCalleeNum));
                pstSCB->szCalleeNum[sizeof(pstSCB->szCalleeNum) - 1] = '\0';
                break;
        }
        ...
        {
            if (sc_dial_make_call2eix(pstSCB, SC_SERV_PREVIEW_DIALING) != DOS_SUCC)
            {
                goto make_all_fail;
            }
        }
    ...
    if (U32_BUTT != ulCustomerID)
    {
        ...
        switch (ulBindType)
        {
            case SC_DID_BIND_TYPE_SIP:
                ...
                /* 判断 sip 分机是否绑定了坐席，如果绑定了坐席，则获得相应的坐席的信息，判断是否需要录音 */
                ...
                if (stAgentInfo.ucStatus != SC_ACD_IDEL)
                {
                    sc_ep_hangup_call(pstSCB, CC_ERR_SC_USER_BUSY);
                    break;
                }
                ...
                {
                    dos_snprintf(szCallString, sizeof(szCallString), "{other_leg_scb=%d,update_agent_id=%d,origination_caller_id_number=%s,origination_caller_id_name=%s,exec_after_bridge_app=park,mark_customer=true}user/%s"
                        , pstSCB->usSCBNo
                        , stAgentInfo.ulSiteID
                        , pstSCB->szCallerNum
                        , pstSCB->szCallerNum
...
U32 sc_ep_num_verify(esl_handle_t *pstHandle, esl_event_t *pstEvent, SC_SCB_ST *pstSCB)
{
        ...
        || DOS_ADDR_INVALID(pstEvent))
    {
        ...
        //sc_ep_esl_execute("hangup", NULL, pstSCB->szUUID);
        sc_ep_hangup_call(pstSCB, CC_ERR_SC_MESSAGE_PARAM_ERR);
...
U32 sc_ep_pots_pro(SC_SCB_ST *pstSCB, BOOL bIsSecondaryDialing)
{
    ...
    /* 判断是否是客户标记 */
    if (bIsSecondaryDialing
        && 1 == dos_sscanf(pszDealNum+1, "%d", &ulKey)
        && ulKey <= 9)
    {
        /* 客户标记 */
...
U32 sc_ep_channel_park_proc(esl_handle_t *pstHandle, esl_event_t *pstEvent, SC_SCB_ST *pstSCB)
{
    ...
    pszValue = esl_event_get_header(pstEvent, "variable_mark_customer_timeout");
    if (DOS_ADDR_VALID(pszValue)
        && dos_strcmp(pszValue, "true") == 0)
    {
        /* 标记完成，挂断电话 */
        ...
        || SC_SERV_OUTBOUND_CALL == ulMainService)
    {
        ...
        */
        ...
        if (DOS_ADDR_INVALID(pstSCBOther))
        {
            ...
            sc_ep_hangup_call(pstSCB, CC_ERR_SC_SYSTEM_ABNORMAL);
            ulRet = DOS_FAIL;
        ...
        {
            sc_ep_hangup_call(pstSCBOther, CC_ERR_SC_SYSTEM_ABNORMAL);
            sc_ep_hangup_call(pstSCB, CC_ERR_SC_SYSTEM_ABNORMAL);
            ulRet = DOS_FAIL;
    ...
    else
    {
        ...
        {
            sc_ep_hangup_call(pstSCB, CC_ERR_SC_NO_SERV_RIGHTS);
        ...
        if (SC_DIRECTION_SIP == ulCallSrc && SC_DIRECTION_PSTN == ulCallDst)
        {
            ...
            {
                sc_ep_hangup_call(pstSCB, CC_ERR_SC_CUSTOMERS_NOT_EXIST);
        ...
        else if (SC_DIRECTION_PSTN == ulCallSrc && SC_DIRECTION_SIP == ulCallDst)
        {
            ...
            if (pstSCB->ulCustomID != U32_BUTT)
            {
                if (sc_send_usr_auth2bs(pstSCB) != DOS_SUCC)
                {
                    sc_ep_hangup_call(pstSCB, CC_ERR_SC_MESSAGE_SENT_ERR);
...
U32 sc_ep_channel_create_proc(esl_handle_t *pstHandle, esl_event_t *pstEvent)
{
    ...
    if (DOS_ADDR_VALID(pszSCBNum))
    {
        ...
        {
            sc_acd_agent_update_status(pstSCB, SC_ACD_BUSY, ulSCBNo);
        }
    ...
    switch (ulStatus)
    {
        ...
        case SC_SCB_RELEASE:
            ...
            if (pstSCB->bIsAgentCall)
            {
                ...
                sc_acd_agent_update_status(pstSCB, ulAgentStatus, U32_BUTT);
                ...
                && !pstSCBOther->bWaitingOtherRelase)
            {
                ...
                sc_logr_debug(SC_ESL, "!!!!!! pstSCB->bIsAgentCall : %u, pstSCBOther->bIsAgentCall : %u, pstSCBOther->bIsMarkCustomer : %u"
                        , pstSCB->bIsAgentCall, pstSCBOther->bIsAgentCall, pstSCBOther->bIsMarkCustomer);
                if (!pstSCB->bIsAgentCall && pstSCBOther->bIsAgentCall && !pstSCBOther->bIsMarkCustomer)
                {
                    /* 获得坐席 */
                    ...
                    else
                    {
                        ...
                        if (sc_call_check_service(pstSCBOther, SC_SERV_AGENT_SIGNIN))
                        {
                            if (stAgentInfo.ucProcesingTime != 0)
                            {
                                ...
                                sc_acd_agent_update_status(pstSCBOther, SC_ACD_PROC, pstSCBOther->usSCBNo);
                            }
                            else
                            {
                                /* 修改状态为空闲状态 */
                                sc_acd_agent_update_status(pstSCBOther, SC_ACD_IDEL, pstSCBOther->usSCBNo);
                            }
                        }
                        else
                        {
                            ...
                            {
                                /* 不要挂断坐席，把坐席置为 整理状态，放音提示坐席，可标记客户 */
                                sc_logr_debug(SC_ESL, "!!!!!! Agent : %u", pstSCBOther->ulAgentID);
                                sc_acd_agent_update_status(pstSCBOther, SC_ACD_PROC, U32_BUTT);
                            }
                            else
                            {
                                pstSCB->bWaitingOtherRelase = DOS_TRUE;
                                sc_acd_agent_update_status(pstSCBOther, SC_ACD_IDEL, U32_BUTT);
                            }
                        }
                    }
                }
