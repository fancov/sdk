...
U32 sc_http_sip_update_proc(U32 ulAction, U32 ulSipID, U32 ulCustomerID)
{
    ...
    switch(ulAction)
    {
        ...
        {
#if INCLUDE_SERVICE_PYTHON
            ulRet = py_exec_func("sip_gen", "sip_add","(i)", ulSipID);
            if (ulRet != DOS_SUCC)
            {
                DOS_ASSERT(0);
                return DOS_FAIL;
...
#if INCLUDE_SERVICE_PYTHON
            ulRet = py_exec_func("sip_gen", "sip_update","(i,i,s)", ulCustomerID, ulSipID, szUserID);
            if (ulRet != DOS_SUCC)
            {
                DOS_ASSERT(0);
                return DOS_FAIL;
...
#if INCLUDE_SERVICE_PYTHON
            ulRet = py_exec_func("sip_gen", "sip_delete","(i,i,s)", ulCustomerID, ulSipID, szUserID);
            if (ulRet != DOS_SUCC)
            {
                DOS_ASSERT(0);
                return DOS_FAIL;
