 ...
 *  ×÷    Õß: Larry
 *  Ãè    Êö: ACDÄ£¿éÏà¹Ø¹¦ÄÜº¯ÊýÊµÏÖ
 *  ÐÞ¸ÄÀúÊ·:
 *
 * ×øÏ¯¹ÜÀíÔÚÄÚ´æÖÐµÄ½á¹¹
 * 1. ÓÉg_pstAgentList hash±íÎ¬»¤ËùÓÐµÄ×øÏ¯
 * 2. ÓÉg_pstGroupList Î¬»¤ËùÓÐµÄ×é£¬Í¬Ê±ÔÚ×éÀïÃæÎ¬»¤Ã¿¸ö×é³ÉÔ±µÄhash±í
 * 3. ×é³ÉÔ±µÄHash±íÖÐÓÐÖ¸Ïòg_pstSiteListÖÐ×øÏ¯³ÉÔ±µÄÖ¸Õë
 *  g_pstAgentList
 ...
 *            |                  ...
 *            ...
 *  ÆäÖÐg_pstGroupList tableÖÐËùÓÐµÄagnetÊ¹ÓÃÖ¸Ïòg_pstAgentListÖÐÄ³Ò»¸ö×øÏ¯,
 *  Í¬Ò»¸ö×øÏ¯¿ÉÄÜÊôÓÚ¶à¸ö×øÏ¯×é£¬ËùÒÔ¿ÉÄÜg_pstGroupListÖÐÓÐ¶à¸ö×øÏ¯Ö¸Ïòg_pstAgentListÖÐÍ¬Ò»¸ö½Úµã£¬
 *  ËùÒÔÉ¾³ýÊ±²»ÄÜÖ±½Ódelete
 */
...
/*
 * º¯  Êý: sc_acd_hash_func4agent
 * ¹¦  ÄÜ: ×øÏ¯µÄhashº¯Êý£¬Í¨¹ý·Ö»úºÅ¼ÆËãÒ»¸öhashÖµ
 * ²Î  Êý:
 *         S8 *pszExension  : ·Ö»úºÅ
 *         U32 *pulHashIndex: Êä³ö²ÎÊý£¬¼ÆËãÖ®ºóµÄhashÖµ
...
/*
 * º¯  Êý: sc_acd_hash_func4grp
 * ¹¦  ÄÜ: ×øÏ¯×éµÄhashº¯Êý£¬Í¨¹ý·Ö»úºÅ¼ÆËãÒ»¸öhashÖµ
 * ²Î  Êý:
 *         U32 ulGrpID  : ×øÏ¯×éID
 *         U32 *pulHashIndex: Êä³ö²ÎÊý£¬¼ÆËãÖ®ºóµÄhashÖµ
 * ·µ»ØÖµ: ³É¹¦·µ»ØDOS_SUCC£¬·ñÔò·µ»ØDOS_FAIL
...
/*
 * º¯  Êý: sc_acd_hash_func4calller_relation
 * ¹¦  ÄÜ: Ö÷½ÐºÅÂëºÍ×øÏ¯¶ÔÓ¦¹ØÏµµÄhashº¯Êý£¬Í¨¹ýÖ÷½ÐºÅÂë¼ÆËãÒ»¸öhashÖµ
 * ²Î  Êý:
 *         U32 ulGrpID  : ×øÏ¯×éID
 *         U32 *pulHashIndex: Êä³ö²ÎÊý£¬¼ÆËãÖ®ºóµÄhashÖµ
 * ·µ»ØÖµ: ³É¹¦·µ»ØDOS_SUCC£¬·ñÔò·µ»ØDOS_FAIL
...
/*
 * º¯  Êý: sc_acd_grp_hash_find
 * ¹¦  ÄÜ: ×øÏ¯×éµÄhash²éÕÒº¯Êý
 * ²Î  Êý:
...
/*
 * º¯  Êý: sc_acd_agent_hash_find
 * ¹¦  ÄÜ: ×øÏ¯µÄhash²éÕÒº¯Êý
 * ²Î  Êý:
...
/*
 * º¯  Êý: S32 sc_acd_site_dll_find(VOID *pSymName, DLL_NODE_S *pNode)
 * ¹¦  ÄÜ: ×øÏ¯µÄhash²éÕÒº¯Êý
 * ²Î  Êý:
 *         VOID *pSymName  : ×øÏ¯·Ö»úºÅ
...
VOID *sc_acd_query_agent_status_task(VOID *ptr)
{
    ...
    while (1)
    {
        ...
        HASH_Scan_Table(g_pstAgentList, ulHashIndex)
        {
            ...
            /* Ò»¸öHASHÍ°×öÒ»´ÎÏß³ÌÇÐ»»(HASH±íÊÇÔ¤·ÖÅäµÄ£¬ËùÒÔ²»Éæ¼°µ½Ïß³ÌÇÐ»»³öÈ¥Ö®ºó£¬Ïß³Ìµã±»ÆÆ»µµÄÎÊÌâ) */
        }
...
U32 sc_acd_agent_update_status_db(U32 ulSiteID, U32 ulStatus, BOOL bIsConnect)
{
    ...
    */
    ...
    if (bIsConnect)
    {
        /* Èç¹ûÊÇ³¤Ç©, Ôò°Ñ×´Ì¬ÐÞ¸ÄÎª³¤Ç©¿ÉÓÃ»òÕß³¤Ç©²»¿ÉÓÃ */
        ulStatusDB += 2;
    }
...
VOID sc_acd_agent_update_status_IDEL(U64 ulArg)
{
    ...
    bIsConnect = pstAgentQueueInfo->pstAgentInfo->bNeedConnected;
...
/*
 * º¯  Êý: U32 sc_acd_agent_update_status(S8 *pszUserID, U32 ulStatus)
 * ¹¦  ÄÜ: ¸üÐÂ×øÏ¯×´Ì¬
 * ²Î  Êý:
...
U32 sc_acd_agent_update_status(U32 ulSiteID, U32 ulStatus, U32 ulSCBNo)
{
    ...
    /* Èç¹û ulStatus Îª SC_ACD_BUTT£¬ Ôò²»¸úÐÂ×´Ì¬ */
...
/*
 * º¯  Êý: U32 sc_acd_agent_grp_del_call(U32 ulGrpID)
 * ¹¦  ÄÜ: ×øÏ¯×éÖÐÓÐµÈ´ý¶ÓÁÐ
 * ²Î  Êý:
 *       U32 ulGrpID :
 * ·µ»ØÖµ: ³É¹¦·µ»ØDOS_SUCC£¬·ñÔò·µ»ØDOS_FAIL
 ...
 * ¹¦  ÄÜ: Í³¼Æ×øÏ¯×øÏ¯×é
 * ²Î  Êý:
 *       U32 ulGrpID : ×øÏ¯×éÖÐÄ³¸öµÈ´ýµÄºô½ÐµÃµ½µ÷¶È£¬Í¨Öª×øÏ¯×é
 * ·µ»ØÖµ: ³É¹¦·µ»ØDOS_SUCC£¬·ñÔò·µ»ØDOS_FAIL
 **/
...
/*
 * º¯  Êý: U32 sc_acd_agent_grp_stat(U32 ulGrpID, U32 ulWaitingTime)
 * ¹¦  ÄÜ: Í³¼Æ×øÏ¯×øÏ¯×é
 * ²Î  Êý:
 *      U32 ulGrpID,       : ×øÏ¯×éID
 *      U32 ulWaitingTime  : ×øÏ¯×éÖÐºô½ÐµÈ´ýÊ±¼ä
 * ·µ»ØÖµ: ³É¹¦·µ»ØDOS_SUCC£¬·ñÔò·µ»ØDOS_FAIL
 **/
...
/*
 * º¯  Êý: U32 sc_acd_agent_stat(U32 ulAgentID, U32 ulCallType, U32 ulStatus)
 * ¹¦  ÄÜ: Í³¼Æ×øÏ¯ºô½ÐÐÅÏ¢
 * ²Î  Êý:
 *       U32 ulAgentID   : ×øÏ¯ID
 *       U32 ulCallType  : ºô½ÐÀàÐÍ
 *       U32 ulStatus    : ×´Ì¬
 * ·µ»ØÖµ: ³É¹¦·µ»ØDOS_SUCC£¬·ñÔò·µ»ØDOS_FAIL
 **/
...
/*
 * º¯  Êý: U32 sc_acd_update_agent_scbno_by_userid(S8 *szUserID, SC_SCB_ST *pstSCB)
 * ¹¦  ÄÜ: ¸ù¾ÝSIP£¬²éÕÒµ½°ó¶¨µÄ×øÏ¯£¬¸üÐÂusSCBNo×Ö¶Î. (ucBindType Îª AGENT_BIND_SIP)
 * ²Î  Êý:
 * ·µ»ØÖµ: ³É¹¦·µ»ØDOS_SUCC£¬·ñÔò·µ»ØDOS_FAIL
 **/
...
/*
 * º¯  Êý: U32 sc_acd_update_agent_scbno_by_siteid(U32 ulAgentID, SC_SCB_ST *pstSCB, SC_AGENT_BIND_TYPE_EN enType)
 * ¹¦  ÄÜ: ÐÞ¸Ä×øÏ¯µÄ×´Ì¬ºÍscbno
 * ²Î  Êý:
 *       SC_DID_BIND_TYPE_EN enType : Èç¹û enType Îª SC_DID_BIND_TYPE_SIP,ÔòÒªÅÐ¶Ï×øÏ¯£¬ÊÇ·ñÕýÔÚÊ¹ÓÃsip£¬
 *                                      Èç¹ûÊ¹ÓÃsip·Ö»ú£¬²ÅÐèÒª¸üÐÂ×øÏ¯µÄ×´Ì¬¡£
 * ·µ»ØÖµ: ³É¹¦·µ»ØDOS_SUCC£¬·ñÔò·µ»ØDOS_FAIL
 **/
...
/*
 * º¯  Êý: U32 sc_acd_group_remove_agent(U32 ulGroupID, S8 *pszUserID)
 * ¹¦  ÄÜ: ´Ó×øÏ¯¶ÓÁÐÖÐÒÆ³ý×øÏ¯
 * ²Î  Êý:
...
/*
 * º¯  Êý: U32 sc_acd_group_add_agent(U32 ulGroupID, SC_ACD_AGENT_INFO_ST *pstAgentInfo)
 * ¹¦  ÄÜ: ½«×øÏ¯Ìí¼Óµ½×øÏ¯¶ÓÁÐ
 * ²Î  Êý:
 *       U32 ulGroupID: ×øÏ¯×éID
 *       SC_ACD_AGENT_INFO_ST *pstAgentInfo : ×øÏ¯×éÐÅÏ¢
 * ·µ»ØÖµ: ³É¹¦·µ»ØDOS_SUCC£¬·ñÔò·µ»ØDOS_FAIL
 **/
...
/*
 * º¯  Êý: sc_acd_add_agent
 * ¹¦  ÄÜ: Ìí¼Ó×øÏ¯
 * ²Î  Êý:
 *         SC_ACD_AGENT_INFO_ST *pstAgentInfo, ×øÏ¯ÐÅÏ¢ÃèÊö
 *         U32 ulGrpID ËùÊô×é
 * ·µ»ØÖµ: SC_ACD_AGENT_QUEUE_NODE_ST *·µ»Ø×øÏ¯¶ÓÁÐÀïÃæµÄ½Úµã¡£±àÒëµ÷ÓÃº¯Êý½«×øÏ¯Ìí¼Óµ½×é¡£Èç¹ûÊ§°ÜÔò·µ»ØNULL
 * !!! ¸Ãº¯ÊýÖ»ÊÇ½«×øÏ¯¼ÓÈë¹ÜÀí¶ÓÁÐ£¬²»»á½«×øÏ¯¼ÓÈë×øÏ¯¶ÔÁÐ !!!
 **/
...
/*
 * º¯  Êý: sc_acd_grp_wolk4delete_site
 * ¹¦  ÄÜ: ´ÓÄ³Ò»¸ö×øÏ¯×éÀïÃæÉ¾³ý×øÏ¯
 * ²Î  Êý:
 *         HASH_NODE_S *pNode : µ±Ç°½Úµã
 *         VOID *pszExtensition : ±»É¾³ý×øÏ¯µÄ·Ö»úºÅ
 * ·µ»ØÖµ: VOID
 **/
static VOID sc_acd_grp_wolk4delete_agent(HASH_NODE_S *pNode, VOID *pulSiteID)
{
    ...
    /* Õâ¸öµØ·½ÏÈ²»Òªfree£¬ÓÐ¿ÉÄÜ±ðµÄ¶ÓÁÐÒ²ÓÐÕâ¸ö×÷Ï¢ */
    pstAgentQueueNode = pstDLLNode->pHandle;
    pstAgentQueueNode->pstAgentInfo = NULL;
...
U32 sc_acd_update_agent_status(U32 ulAction, U32 ulAgentID)
{
    ...
    switch (ulAction)
    {
        ...
        case SC_ACD_SITE_ACTION_CONNECTED:
            ...
            {
                /* ³¤Ç©µÄÊ±ºò£¬Èç¹û×øÏ¯×´Ì¬Îª SC_ACD_OFFLINE£¬ Ôò¸üÐÂÎª SC_ACD_AWAY */
                pstAgentQueueInfo->pstAgentInfo->ucStatus = SC_ACD_AWAY;
            }
    ...
    if (bIsUpdateDB)
    {
        sc_acd_agent_update_status_db(ulAgentID, pstAgentQueueInfo->pstAgentInfo->ucStatus, pstAgentQueueInfo->pstAgentInfo->bNeedConnected);
    }
...
SC_ACD_AGENT_QUEUE_NODE_ST * sc_acd_get_agent_by_random(SC_ACD_GRP_HASH_NODE_ST *pstGroupListNode)
{
    ...
    /*
     * Ëæ»úÒ»¸ö±àºÅ£¬È»ºó´ÓÕâ¸ö±àºÅ¿ªÊ¼²éÕÒÒ»¸ö¿ÉÓÃµÄ×øÏ¯¡£Èç¹ûµ½¶ÓÎ²ÁË»¹Ã»ÓÐÕÒµ½¾ÍÔÙ´ÓÍ·À´
     */
    ...
    if (DOS_ADDR_INVALID(pstAgentInfo))
    {
        ...
        DLL_Scan(&pstGroupListNode->stAgentList, pstDLLNode, DLL_NODE_S*)
        {
            ...
            /* ååµ½ÕâÀïÒÑ¾­²éÕÒ¹ýËùÓÐµÄµÄ×øÏ¯ÁË */
            if (pstAgentQueueNode->ulID > ulRandomAgent)
            {
...
SC_ACD_AGENT_QUEUE_NODE_ST * sc_acd_get_agent_by_inorder(SC_ACD_GRP_HASH_NODE_ST *pstGroupListNode)
{
    ...
    /*
     * ´ÓÉÏ´ÎÊ¹ÓÃµÄ±àºÅ¿ªÊ¼²éÕÒÒ»¸ö¿ÉÓÃµÄ×øÏ¯¡£Èç¹ûµ½¶ÓÎ²ÁË»¹Ã»ÓÐÕÒµ½¾ÍÔÙ´ÓÍ·À´
     */
...
start_find:
    ...
    DLL_Scan(&pstGroupListNode->stAgentList, pstDLLNode, DLL_NODE_S*)
    {
        ...
        /* ÕÒµ½Ò»¸ö±È×îºóÒ»¸ö´óÇÒ×îÐ¡¹¤ºÅµÄ×øÏ¯ */
    ...
    {
        /* Ã»ÓÐÕÒµ½µ½£¬ÕÒ×îÐ¡µÄ¹¤ºÅ±àºÅµÄ×øÏ¯À´½Óµç»° */
        szLastEmpNo[0] = '\0';
...
SC_ACD_AGENT_QUEUE_NODE_ST * sc_acd_get_agent_by_caller(SC_ACD_GRP_HASH_NODE_ST *pstGroupListNode, S8 *szCallerNum)
{
    ...
    /* ¸ù¾Ý×øÏ¯±àºÅ, ÔÚ×øÏ¯hashÖÐ²éÕÒµ½¶ÔÓ¦µÄ×øÏ¯ */
...
process_call:
    ...
    {
        /* Ìí¼Ó»òÕß¸üÐÂ Ö÷½ÐºÅÂëºÍ×øÏ¯¶ÔÓ¦¹ØÏµµÄhash */
        sc_logr_notice(SC_ACD, "Found an uaeable agent. Call Count: %d. (Agent %d in Group %d)"
                        , pstAgentNode->pstAgentInfo->ulCallCnt
...
static S32 sc_acd_init_agent_queue_cb(VOID *PTR, S32 lCount, S8 **pszData, S8 **pszField)
{
    ...
    /* ²é¿´µ±Ç°ÒªÌí¼ÓµÄ×øÏ¯ÊÇ·ñÒÑ¾­´æÔÚ£¬Èç¹û´æÔÚ£¬¾Í×¼±¸¸üÐÂ¾ÍºÃ */
        ...
        && DOS_ADDR_VALID(pstHashNode->pHandle))
    {
        ...
        {
            /* ¿´¿´×øÏ¯ÓÐÃ»ÓÐÈ¥ÁË±ðµÄ×é£¬Èç¹ûÊÇ£¬¾ÍÐèÒª½«×øÏ¯»»µ½±ðµÄ×é */
            for (ulIndex = 0; ulIndex < MAX_GROUP_PER_SITE; ulIndex++)
            {
                ...
                {
                    /* ÐÞ¸ÄÖ®Ç°×éIDºÏ·¨£¬ÐÞ¸ÄÖ®ºó×éIDºÏ·¨£¬¾ÍÐèÒª¿´¿´Ç°ºóÁ½¸öIDÊÇ·ñÏàÍ¬£¬ÏàÍ¬¾Í²»×öÊ²Ã´ÁË */
                    ...
                    /* ÐÞ¸ÄÖ®Ç°×éIDºÏ·¨£¬ÐÞ¸ÄÖ®ºó×éID²»ºÏ·¨£¬¾ÍÐèÒª°Éagent´ÓÖ®Ç°µÄ×éÀïÃæÉ¾³ýµô */
                    else
                    {
                        sc_logr_debug(SC_ACD, "Agent %u will be removed from group %u."
                                        , pstAgentQueueNode->pstAgentInfo->ulSiteID
                ...
                {
                    /* ÐÞ¸ÄÖ®Ç°×éID²»ºÏ·¨£¬ÐÞ¸ÄÖ®ºó×éIDºÏ·¨£¬¾ÍÐèÒª°ÉagentÌí¼Óµ½ËùÉèÖÃµÄ×é */
                    if (stSiteInfo.aulGroupID[ulIndex] != U32_BUTT
                        && stSiteInfo.aulGroupID[ulIndex] != 0)
                    {
                        /* Ìí¼Óµ½ÐÂµÄ×é */
                    ...
                    {
                        /* ÐÞ¸ÄÖ®Ç°×éID²»ºÏ·¨£¬ÐÞ¸ÄÖ®ºó×éID²»ºÏ·¨£¬¾ÍÉ¶Ò²²»×öÁ¨ */
                    }
                }
            }
...
/**
 * º¯Êý: U32 sc_acd_http_agent_update_proc(U32 ulAction, U32 ulAgentID, S8 *pszUserID)
 * ¹¦ÄÜ: ´¦ÀíHTTP·¢¹ýÀ´µÄÃüÁî
 * ²ÎÊý:
 *      U32 ulAction : ÃüÁî
 *      U32 ulAgentID : ×øÏ¯ID
