...
def config_operation(ulOpObj, ulOpType, ulObjID, seqParam1, seqParam2):
    ...
    if str(ulOpObj).strip() == '':
        print file_info.get_file_name(),file_info.get_line_number(),file_info.get_function_name(),'op_obj is', ulOpObj
        return None 
    if str(ulOpType).strip() == '':
        print file_info.get_file_name(),file_info.get_line_number(),file_info.get_function_name(),'op_type is', ulOpType
        return None
    if str(ulObjID).strip() == '':
        print file_info.get_file_name(),file_info.get_line_number(),file_info.get_function_name(),'obj_id is', ulObjID
        return None
    ...
    # 如果op_type值为2，则表示删除操作
        ...
        # 如果op_obj为2，则表示删除一个客户，那么param2必须为"delete"，参数param1必须为None，obj_id为客户id  
            ...
            if seqParam2.lower().strip() == 'delete' and seqParam1 == None:
                customer_mgnt.del_customer(ulObjID)  # 测试成功
                return
    # 如果op_type值是3，那么表示更新操作          
        ...
        # 如果op_obj为1，则表示更新group相关内容
            ...
            if seqParam2.lower().strip() == 'change':
                ulNewGroupID = int(seqParam1)
                group_mgnt.modify_group_name(ulObjID, ulNewGroupID) # 测试成功
                return
